...
return new class extends Migration
{
    ...
    public function up()
    {
        DB::unprepared("CREATE TRIGGER after_user_balance_histories_insert
        AFTER INSERT
        ...
        BEGIN
            ...
            IF NOT EXISTS(select 1 from user_balances where user_id = NEW.user_id) THEN
                insert into user_balances(`user_id`, `rovi_balance`,`dev3_balance`,`usdt_balance`)  values(NEW.user_id, 0,0,0) ;
            END IF;
    ...
    {
        DB::unprepared('DROP TRIGGER `after_user_balance_histories_insert`');
    }
};
